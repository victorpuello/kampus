...
import { academicApi, type EditRequest, type Period } from '../services/academic'
...
export default function PlanningEditRequests() {
  ...
  const refreshMyRequests = async () => {
    ...
    try {
      ...
      setMyRequests(res.data.filter((r) => r.scope === 'PLANNING').sort((a, b) => (a.created_at < b.created_at ? 1 : -1)))
    } catch {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        await loadContext()
        if (!mounted) return
        if (isTeacher) await refreshMyRequests()
        if (isAdminLike) await refreshPendingRequests()
      } catch {
  ...
  if (!isTeacher && !isAdminLike) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Solicitudes de edición (Planeación)</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                disabled={submitting}
                placeholder="Describe por qué necesitas reabrir la edición de planeación…"
              />
            </div>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {loadingMyRequests ? (
                    <tr className="bg-white dark:bg-slate-900/40">
                      <td className="px-6 py-6 text-slate-600 dark:text-slate-300" colSpan={4}>Cargando…</td>
                    </tr>
                  ) : myRequests.length === 0 ? (
                    <tr className="bg-white dark:bg-slate-900/40">
                      <td className="px-6 py-6 text-slate-600 dark:text-slate-300" colSpan={4}>No has creado solicitudes.</td>
                    </tr>
                  ) : (
                    myRequests.map((r) => {
                      const statusClass =
          ...
          <CardContent>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {loadingPending ? (
                    <tr className="bg-white dark:bg-slate-900/40"><td className="px-6 py-6 text-slate-600 dark:text-slate-300" colSpan={6}>Cargando…</td></tr>
                  ) : pendingRequests.length === 0 ? (
                    <tr className="bg-white dark:bg-slate-900/40"><td className="px-6 py-6 text-slate-600 dark:text-slate-300" colSpan={6}>No hay solicitudes pendientes.</td></tr>
                  ) : (
                    pendingRequests.map((r) => {
                      const decision = decisionById[r.id] || { valid_until: '', decision_note: '', submitting: false }
                      const disabled = !!decision.submitting
